...
function buildChatGptAnswerPrompt(company, prompt, competitors, analysis) {
  const context = {
    selected_company: {
  ...
  return `Answer this buyer research prompt as ChatGPT would answer it.
...
async function analyzePromptVisibility(company, prompts, competitors, analysis) {
  ...
  for (const prompt of prompts) {
    ...
    try {
      const payload = await callOpenAi(buildChatGptAnswerPrompt(company, prompt, competitors, analysis), controller.signal);
