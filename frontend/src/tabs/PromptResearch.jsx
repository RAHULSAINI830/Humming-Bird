...
export default function PromptResearch({ data, onChange, workspace, goTo }) {
  ...
  async function researchPrompts(event) {
    ...
    try {
      ...
      if (result.limits && data) {
        onChange({ ...data, limits: result.limits });
      }
  ...
  async function includeSelectedPrompts() {
    ...
    try {
      ...
      setSuggestions([]);
      setSelected({});
      setMessage(`${selectedPrompts.length} researched prompt${selectedPrompts.length === 1 ? '' : 's'} added to the prompt table.`);
    ...
    <section className="page-content prompt-research-page">
      ...
      <article className="prompt-research-hero">
        <div>
          ...
          <p>Hummingbird uses the saved business profile, existing prompts, and your brief to suggest non-duplicate prompts that fit the remaining workspace capacity.</p>
      ...
      <div className="metric-grid prompt-research-metrics">
        ...
        <Metric title="Selected" value={selectedCount} helper="Prompts ready to add" />
      </div>
      ...
      <div className="prompt-research-layout">
        <article className="prompt-research-workbench">
          <div className="dashboard-panel-head">
            <div>
              ...
              <h2>Tell Hummingbird what prompt set you need</h2>
            </div>
          ...
          <form className="prompt-research-page-form" onSubmit={researchPrompts}>
            ...
            <button className="primary-button" type="submit" disabled={researching || remaining <= 0 || !canManage}>
              {researching ? 'Researching prompts…' : `Research up to ${remaining} prompts`}
            </button>
          </form>
        </article>
        ...
        <article className="prompt-research-results-panel">
          <div className="dashboard-panel-head">
            ...
            <span>{selectedCount}/{remaining} selected</span>
          </div>
